...
public class CameraImplementedDAO  extends CameraDAO{
	
	
	CameraDTO[] superCameraArray=super.getcameraDTOs();
	int adder=0;
	
	 public CameraImplementedDAO() {
	...
	public CameraDTO findByMaxCost() {
		...
		for(int i=0;i<this.superCameraArray.length;i++)
		{
			...
			{
				double cost=superCameraArray[i].getPrice();
				if(cost>maxCost)
				{
					maxCost=cost;
					maxCostIndex=i;
	...
	public CameraDTO findByMinCost() {
		...
		double minCost=superCameraArray[0].getPrice();
		int minCostIndex=0;
